...
const clamp = (num: number, min = -20, max = 20) =>
...
export function Card(props: Props) {
  ...
  const interact = (e: any) => {
    ...
    const stylePersonalize = {
      ...
      "--hyp": `${
        Math.sqrt(
          (springGlareY.val - 50) * (springGlareY.val - 50) +
            (springGlareX.val - 50) * (springGlareX.val - 50)
        ) / 50
      }`,
    };
  ...
  useEffect(() => {
    var r = document.querySelector<HTMLElement>(`#${id}`);
    if (activeCard) {
      const div = document.createElement("div");

      div.className = styles.cardShine;
      div.id = styles.cardShine;
      r?.appendChild(div);
    } else {
      ...
      >
        <FrontSide animationDuration={2000}>
          ...
          >
            <CardAnimated
              ...
              borderRadius="5px"
            >
              <div className={styles.frontSide} id={id}>
        ...
        <BackSide animationDuration={2000}>
          ...
          >
            ...
            >
              <img
                src="https://www.svgrepo.com/show/109426/gym-near.svg"
                alt="gym"
              />
            </div>
          </CardAnimated>
